...
const restaurants: Restaurant[] = [
  {
    ...
    "name": "TailorHub",
    "neighborhood": "Madrid",
    "photograph": "tailor-hub.jpg",
    "address": "C/ Hermanos Álvarez Quintero, 2, 28004 Madrid",
    "latlng": {
      "lat": 40.713829,
      "lng": -73.989667
    },
    "image": "/tailor-hub.jpg",
    "cuisine_type": "Beers",
    "operating_hours": {
    ...
    "reviews": [{
      "name": "Adrian Martin",
      "date": "October 06, 1990",
      "rating": 4.9,
      "comments": "TailorHub no es solo un restaurante; es como una aplicación bien diseñada donde cada plato refleja una interfaz limpia y cada sabor es una funcionalidad perfectamente ejecutada. Desde su evolución desde los humildes días en Madrid hasta convertirse en un vasto restaurante de dos pisos con tecnología punta en la cocina, TailorHub ha demostrado cómo la innovación y atención al detalle pueden elevar la experiencia culinaria al nivel de arte. Como desarrollador, estoy agradecido por la oportunidad de intentar formar parte de un equipo que valora la creatividad y la precisión tanto en su cocina como yo en mi código."
    },
    ]
  },
  {
...
const api = {
  ...
  create: async (newRestaurant: Restaurant): Promise<void> => {
    await sleep(750);
    restaurants.push(newRestaurant);
  },
  update: async (id: Restaurant["id"], updatedRestaurant: Restaurant): Promise<void> => {
    await sleep(750);
    const index = restaurants.findIndex((restaurant) => restaurant.id === id);
    if (index === -1) {
      throw new Error(`Restaurant with id ${id} not found`);
    }
    restaurants[index] = { ...updatedRestaurant, id };
  },
  delete: async (id: Restaurant["id"]): Promise<void> => {
    await sleep(750);
    const index = restaurants.findIndex((restaurant) => restaurant.id === id);
    if (index === -1) {
      throw new Error(`Restaurant with id ${id} not found`);
